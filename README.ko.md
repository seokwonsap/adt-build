...
총 16종의 ABAP 객체를 지원하며, RAP 서비스를 즉시 호출 가능한 OData V4로 배포하는 작업도 완벽히 지원합니다.
...
**이 도구의 진가는 온프레미스 AS ABAP 환경에서 발휘됩니다.** SAP의 공식 ADT-for-VS-Code / MCP는 ABAP Cloud 전용입니다. 따라서 기존 온프레미스 시스템에서 AI 에이전트가 객체를 직접 만들고 활성화할 'Write' 수단은 현재 이 도구가 거의 유일합니다. Eclipse도, Cloud도, MCP도 필요 없습니다.
...
- **ADT MCP 서버와의 결합:** adt-build는 빌드를 전담하고, 읽기/조회/대화형 수정은 커뮤니티 ADT MCP 서버(VSP 등)에 맡기면 역할을 깔끔하게 분리할 수 있습니다. 시스템 제약으로 MCP 연동이 막힌 환경에서는 이 Raw-REST 빌더가 훌륭한 대안(Fallback)이 됩니다.

**기존 도구와의 차이점:**

- **abapGit:** 기존에 존재하는 객체를 Git으로 직렬화하고 이송하는 목적입니다. adt-build는 로컬 소스 파일을 ADT REST를 통해 직접 시스템에 '빌드'하는 도구로 결이 다릅니다.
